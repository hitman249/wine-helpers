...
9) Если требуется `winetricks` используйте команду `./start winetricks wmp9`

10) Если нужен `dxvk` ставите в конфиге `dxvk = 1` и `dxvk_autoupdate = 1`, в этом случае актуальный dxvk скачается сам, 
и будет автоматически обновляться при появлении более новой версии!
...
* Папку **wine** лучше всегда упаковывать командой `./start pack wine`, появится файл `wine.squashfs`, папку **wine** 
нужно удалить. Тем самым вы сэкономите 400 мб места.
